  ...
  def create_index(self):
                    ...
                    "properties": {
                      ...
                      "metadata": {
                        "properties": {
                            ...
                            "app_size": {"type": "number"}  # Changed to number for app size
                        }
                      },
                      "privacylabels": {
                        "properties": {
    ...
    try:
      if not self.es.indices.exists(index=self.index_name):
        self.es.indices.create(index=self.index_name, mappings=apple_mapping)
    ...
    with open(filename, 'r') as file:
      for line in file:
        try:
          ...
          if 'version_release_date' in item['metadata'] and item['metadata']['version_release_date'] == '':
            item['metadata']['version_release_date'] == None
            print(item['metadata'])
          
          if 'release_date' in item['metadata'] and item['metadata']['release_date'] == '':
            item['metadata']['release_date'] == None
            print(item['metadata'])
...
def main():
  ...
  for filename in sorted(os.listdir()):
    print(filename)
    if index < 10:
      apple_elastic_handler = AppleElasticSearchHandler(server_url, f'run_0000{index}', username, password)
    else:
      apple_elastic_handler = AppleElasticSearchHandler(server_url, f'run_000{index}', username, password)
    ...
    apple_elastic_handler.stream_and_update(f'run{index}.json')
    index += 1
