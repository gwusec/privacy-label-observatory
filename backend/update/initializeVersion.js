...
const indexName = 'version_release';
...
async function versionHelper(value, run) {
    const labelMap = {
    ...
    for (const rating of contentRatings) {
        try {
                ...
                query: {
                    bool: {
                        must: [
                            {
                                term: {
                                    "privacylabels.privacyDetails.privacyTypes.privacyType.keyword": label
                                }
                            },
                            {
                                term: {
                                    "metadata.content_rating.keyword": rating
                                }
                            }
                        ]
                    }
            ...
            result[rating.replace("+", "")] = response.count;
        } catch (error) {
            console.error(`Error fetching count for ${label} and rating ${rating}:`, error);
            result[rating.replace("+", "")] = 0;
        }
    }
...
async function getVersion() {
    let totals = await getTotal((await getLatestRunIndex()));
    percentages = {}

    //This is for all percentages of data not collected
    var dnc_totals = await versionHelper("dnc", (await getLatestRunIndex()));
    ...
    var dnlty_totals = await versionHelper("dnlty", (await getLatestRunIndex()));
    ...
    var dlty_totals = await versionHelper("dlty", (await getLatestRunIndex()));
    ...
    var duty_totals = await versionHelper("duty", (await getLatestRunIndex()));
...
uploadRatio()
    .then(() => {
        console.log("Version Release Upload Successful");
    })
    .catch((err) => {
