...
#Beginning of migrate.py
def pullApps(run_id, iterator):
    url = 'http://localhost:8000/database/' + str(run_id) + '/' + str(iterator)
    try:
...
def writeGraphData():
    for i in range(1, 70):
        data = json.dumps(buildDataJson(i))
        graphData = open(f"./runs/run{i}.json", "w")
...
#Beginning of importJson.py
def buildDataJson():
    with open('run6.json', 'r') as file:
        data = json.load(file)
    ...
    graphData = open("testrun6.json", "w")
...
#Beginning of elasticTransfer.py
